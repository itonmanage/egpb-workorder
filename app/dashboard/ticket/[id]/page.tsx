...
function TicketDetailsContent({ ticketId }: { ticketId: string }) {
    ...
    const fetchTicket = useCallback(async () => {
        try {
            ...
            if (sessionResult.success && sessionResult.data?.user) {
                ...
                setIsAdmin(userRole === 'ADMIN' || userRole === 'IT_ADMIN');
            }

            // Fetch ticket using API
            const result = await apiClient.tickets.get(ticketId);
    ...
    const handleAdminImageUpload = async () => {
        ...
        try {
            ...
            const result = await uploadTicketImages(ticketId, adminImages, true);
    ...
    const confirmDeleteImage = async () => {
        ...
        try {
            ...
            const deletePromises = imagesToDelete.map(async (imageId) => {
                try {
                    const response = await fetch(`/api/tickets/${ticketId}/images/${imageId}`, {
                        method: 'DELETE',
                        headers: {
    ...
    return (
        <div className="min-h-screen bg-white p-3 sm:p-4 lg:p-8">
            <div className="max-w-4xl mx-auto">
                <Link href="/dashboard" className="inline-flex items-center text-sm text-black hover:text-green-600 mb-4 sm:mb-6 transition-colors">
                    <ArrowLeft size={16} className="mr-1" />
                    Back to Dashboard
                </Link>

                <div className="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-green-100 overflow-hidden">
                    {/* Header */}
                    <div className="p-3 sm:p-4 lg:p-8 border-b border-green-100 bg-green-50">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-4">
                            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                                <span className="text-xs sm:text-sm font-mono text-black bg-white px-2 py-1 rounded border border-green-200">
                    ...
                    {/* Content */}
                    <div className="p-3 sm:p-4 lg:p-8 grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                        {/* Main Info */}
                        <div className="lg:col-span-2 space-y-4 sm:space-y-6 lg:space-y-8">
                            <div>
                                <h3 className="text-sm font-medium text-gray-700 lg:font-semibold lg:text-black lg:uppercase lg:tracking-wider mb-3">Description</h3>
                                <div className="prose max-w-none text-black bg-white lg:bg-green-50 p-4 sm:p-6 lg:p-6 rounded-xl sm:rounded-2xl border border-gray-200 lg:border-green-100">
                            ...
                            <div>
                                <h3 className="text-sm font-medium text-gray-700 lg:font-semibold lg:text-black lg:uppercase lg:tracking-wider mb-3">Attached Images</h3>
                                <div className="bg-white lg:bg-green-50 p-4 sm:p-6 lg:p-6 rounded-xl sm:rounded-2xl border border-gray-200 lg:border-green-100">
                                    {userImages.length > 0 ? (
                            ...
                            {isAdmin && (
                                <div>
                                    <h3 className="text-sm font-medium text-gray-700 lg:font-semibold lg:text-black lg:uppercase lg:tracking-wider mb-3">Admin - Upload Completion Images</h3>
                                    <div className="bg-white p-4 sm:p-6 lg:p-6 rounded-xl sm:rounded-2xl border border-gray-200 lg:border-green-200 lg:shadow-sm">
                                        {/* Upload Area */}
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                                                    {adminImages.map((file, index) => (
                                                        <div key={index} className="relative group">
                                                            <div className="aspect-square w-full bg-gray-100 rounded-lg border-2 border-green-200 overflow-hidden relative">
                                                                <NextImage
                        ...
                        <div className="space-y-6">
                            <div className="bg-white lg:bg-green-50 p-4 sm:p-6 lg:p-6 rounded-xl sm:rounded-2xl border border-gray-200 lg:border-green-100">
                                <h3 className="text-sm font-medium text-gray-700 lg:font-semibold lg:text-black lg:uppercase lg:tracking-wider mb-4">Details</h3>
                                        ...
                                        {isAdmin ? (
                                            <>
                                                ...
                                                <div className="flex items-center justify-between mt-4">
                                                    ...
                                                    <button
                                                        onClick={async () => {
                                                            // Validate that Assign To is filled
                                                            // if (!assignTo || assignTo.trim() === '') {
                                                            //     alert('Please fill in the "Assign To" field before saving.');
                                                            //     return;
                                                            // }
                                                            ...
                                                            if (!assignToChanged && !notesChanged) {
                                                                return;
                                                            }

                                                            setSavingNotes(true);
                                                            try {
                                                                const result = await apiClient.tickets.update(ticket.id, {
                                                                    assignTo: assignTo,
                                                                    adminNotes: adminNotes,
                                                                });
                ...
                {/* Activity Timeline */}
                {ticket && <ActivityTimeline ticketId={ticket.id} ticketType="it" />}
